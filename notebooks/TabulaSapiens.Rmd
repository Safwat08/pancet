...
# Benchmark decontamination using decontx
# Step1. Generate EC atlas with decontx counts
...
counts <- counts$decontXcounts

# Create new object
object <- CreateSeuratObject(counts = counts, meta.data = meta)

# Save previous object
object2 <- tabulasapiens_ecatlas

# Normalize
object <- NormalizeData(object)

object <- SetIdent(object, value = 'organ_tissue')

# Make into new tabulasapiens_ecatlas
tabulasapiens_ecatlas <- object
```
...
object <- RunPCA(object, assay = 'LOG', features = c(present_genes))
object <- FindNeighbors(object, reduction = 'pca', dims = 1:30)
object <- FindClusters(object, resolution = 0.2)
object <- RunVarimaxPCA(object, features = c(present_genes), reduction = 'pca', norm = 'LOG', ncomp = 30, scaleby_sdev = F)
...
pca_df <- FetchData(object, vars = c(meta, 'seurat_clusters', 'VPC_3', 'VPC_8')) %>%
  mutate(clusters = factor(seurat_clusters, levels = as.numeric(names(colors_bp)))) %>%
  mutate(colors = as.vector(colors_bp[match(.$seurat_clusters, data$clusters)]))

colors_vp <- pca_df %>%
  pull(colors, clusters)

p <- ggplot(pca_df, aes(x=VPC_3, y=VPC_8, color = clusters)) +
    geom_point(size = 1) +
    scale_color_manual(values = colors_vp) + 
  theme_classic() +
...
p <- ggplot(pca_df_panc, aes(x=VPC_3, y=VPC_8, color = clusters)) +
    geom_point(size = 1) +
    scale_color_manual(values = colors_vp) + 
  theme_classic() +
    theme(panel.border = element_rect(color = "black", fill = NA, size = 1),
...
ggsave(p, filename = '../figures/tabulasapiens_vpca_plot_pancreas.png', 
       width = 4, height = 4)

ggsave(p, filename = '../figures/tabulasapiens_vpca_plot_pancreas.svg', 
       width = 4, height = 3.5)
...
pca_df <- object@reductions[['vpca']]@feature.loadings %>%
  ...
  mutate(genes = rownames(.),
         label = ifelse(rank(-VPC_3) <= 5 | 
                          rank(-VPC_8) <= 5, genes, NA))

p <- ggplot(pca_df, aes(x=VPC_3, y=VPC_8, label = label)) +
...
ggsave(plot = p, filename = '../figures/tabulasapiens_ecatlas_signature_genes_vpcafeatures.svg', width = 3.5, height = 3.5)
```
...
ggsave(dPlot, filename = '../figures/tabulasapiens_ecatlas_s1pgenes_dotplot.svg', width = 5, height = 6)
```
...
saveRDS(human_only_genes, file = '../objects/signature_genes_humanonly.rds')
...
saveRDS(adult_only_genes, file = '../objects/signature_genes_adultonly.rds')
